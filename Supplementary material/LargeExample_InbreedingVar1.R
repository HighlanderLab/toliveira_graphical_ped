...
tmp <- estimatesInbreedingVar1 <- data_cost <- ESS <- list()
for (ii in 1:6) {
  #---------------------------------------------------------------------
  # Getting the data
  #---------------------------------------------------------------------
  ...
  (time_PedCholVar1_mean <- mean(time))
  (time_PedCholVar1_se <- sd(time)/sqrt(nt))

  # coda samples - MCMC
  coda_samples <- as.mcmc.list(runJagsOut)

  # parameter estimates
  estimatesInbreedingVar1[[ii]] <- MCMCsummary(coda_samples, round = 4)

  #---------------------------------------------------------------------
  # Computational cost
  #---------------------------------------------------------------------
  neef <- estimatesInbreedingVar1[[ii]]$n.eff
  cost <- as.numeric(time_PedCholVar1_mean)/neef
  #---------------------------------------------------------------------
  # Summary
  #---------------------------------------------------------------------
  data_cost[[ii]] <- data.frame(time_PedCholVar1_mean, time_PedCholVar1_se,
                                neef, cost)
  ...
  data_cost[[ii]]$par <- gsub("[^a-zA-Z]", "",
                              rownames(estimatesInbreedingVar1[[ii]]))
...
data_cost_CholVar1 <- do.call(rbind.data.frame, data_cost)
data_cost_CholVar1$model <- "Known Variance"

ESS_CholVar1 <- do.call(rbind.data.frame, ESS)
ESS_CholVar1$model <- "Known Variance"


save(estimatesInbreedingVar1, data_cost_CholVar1,
     file = "./Results/Large_Example/DataCostCholVar1.RData")
saveRDS(ESS_CholVar1, file = "./Results/Large_Example/ESSCholVar1.RDS")
...
tmp <- estimatesInbreedingVar1Var <- data_cost <- ESS <- list()
for (ii in 1:6) {
  #---------------------------------------------------------------------
  # Getting the data
  #---------------------------------------------------------------------
  ...
  # parameter estimates
  estimatesInbreedingVar1Var[[ii]] <- MCMCsummary(coda_samples, round = 4)

  #---------------------------------------------------------------------
  # Computational cost
  #---------------------------------------------------------------------
  neef <- estimatesInbreedingVar1Var[[ii]]$n.eff
  cost <- as.numeric(time_PedCholVar1_mean)/neef
  #---------------------------------------------------------------------
  # Summary
  #---------------------------------------------------------------------
  ...
  data_cost[[ii]]$Generation <- ii + 4
  data_cost[[ii]]$par <- gsub("[^a-zA-Z]", "",
                              rownames(estimatesInbreedingVar1Var[[ii]]))
...
data_cost_CholVar1Var <- do.call(rbind.data.frame, data_cost)
data_cost_CholVar1Var$model <- "Unknown Variance"

ESS_CholVar1Var <- do.call(rbind.data.frame, ESS)
ESS_CholVar1Var$model <- "Unknown Variance"

save(estimatesInbreedingVar1Var, data_cost_CholVar1Var,
     file = "./Results/Large_Example/DataCostCholVar1Var.RData")
saveRDS(ESS_CholVar1Var, file = "./Results/Large_Example/ESSCholVar1Var.RDS")
...
pdf("FigureCholVar1",  width = 5,  height = 3)
